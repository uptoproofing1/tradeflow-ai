...
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// POST /api/auth/signup — create Supabase user + start trial
// ═══════════════════════════════════════════════════════════
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    if (authData.error) return res.status(400).json({ error: authData.error.message || 'Signup failed' });

    const userId = authData.user?.id;
    const token  = authData.access_token;

    // Save profile + trial start in Supabase DB
    if (userId) {
...
// ═══════════════════════════════════════════════════════════
// POST /api/auth/login — login + check subscription
// ═══════════════════════════════════════════════════════════
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const authData = await authRes.json();
    if (authData.error) return res.status(401).json({ error: 'Invalid email or password' });
    ...
    // Work out subscription status
    let subStatus = profile.sub_status || 'trial';
    const trialStart = profile.trial_start ? new Date(profile.trial_start).getTime() : Date.now();
    const daysSinceTrial = Math.floor((Date.now() - trialStart) / (1000 * 60 * 60 * 24));
    if (subStatus === 'trial' && daysSinceTrial >= 7) subStatus = 'expired';

    res.json({
      token,
      user: { id: userId, email, name: profile.name || '', company: profile.company || '' },
      subStatus,
      trialStart,
    });
  } catch (e) {
